...
<body>
	<div class="header">
		...
		<div id="side-menu" class="side-nav">
			...
			<a class="nav-link active" href="data_penghuni_kost.php">Data Penghuni Kost</a>
					 ...
					 id="frm_input">
						<div class="form-group">
							...
							<select class="form-control" name="pemilik" id="pemilik">
								<option selected value="0">--pilih pemilik kost--</option>
								<?php
include 'config/koneksi.php';
$sql = mysqli_query($conn, "SELECT * FROM kost GROUP BY nik");
while ($row = mysqli_fetch_assoc($sql)) {
    echo '<option value="' . $row['pemilik'] . '">
						...
						<div class="form-group nama-kost" id="frm_input">
							<label>Nama kost :</label>
							<select class="form-control" name="nama_kost" id="nama_kost">
							</select>
						</div>
						<a href="report/report_penghuni_kost.php?nama_kost=$nama_kost&pemilik=$pemilik"><button type="submit" class="btn btn-success"><i
								 class="fa fa-search"></i>download</button></a>
					</form>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
      ...
      $(".hapus").click(function () {
		...
		function getPagination() {
			var pageJson = 'http://abu.lineupdev.com/tegalparang/config/getDataPenghuniKostJson.php?page=';
		...
		function clickPagination(item) {
			...
			var nama_kost = $('#nama_kost').val();
			var dataString = 'nama_kost=' + nama_kost + '&page=' + page;
			$.ajax({
				type: "POST",
				url: "config/getDataPenghuniKost.php",
				data: dataString,
