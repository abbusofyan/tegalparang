<?php
  ...
  if ($_POST['u_id']) {
    $u_id = $_POST['u_id'];
    $query = mysqli_query($conn, "SELECT * FROM kost WHERE u_id = '$u_id'");
      ...
      while ($row = mysqli_fetch_assoc($query)) {
        ...
        <?php
          ...
          if ($row['no_izin_kost'] == "") {
            echo "
            <div class='form-group'  id='no_imb'>
              <input id='input_no_izin_kost' type='text' class='form-control' value='Tidak memiliki izin kost' disabled>
            </div>
            ";
          }else{

            echo "
            <p>Memiliki Izin rumah kost</p>
            <div class='form-group'  id='no_izin_kontrakan'>
              <label>No. Izin kontrakan</label>
              <input id='input_no_kontrakan' type='text' value=".$row['no_izin_kost']."  class='form-control' disabled>
            </div>";
          }
         ?>
