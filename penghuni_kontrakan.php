...
<body>
	<div class="header">
		...
		<div id="side-menu" class="side-nav">
			...
			<a class="nav-link active" href="penghuni_kontrakan.php">Input Data penghuni kontrakan</a>
	...
	<div class="row">
		<div class="container">
			<h3 class="title">Data penghuni kontrakan di Kel. Tegal parang</h3>
			</br>
		</div>
	</div>
		...
		<div class="row">
			<div class="container">
				<div class="col-sm-12 col-lg-12">
					<form method="post" class="needs-validation" novalidate id="frm_input" action="test.php" enctype="multipart/form-data">
						<div class="form-group">
							<label>Pemilik kontrakan :</label>
							<?php
include 'config/koneksi.php';
$query = mysqli_query($conn, "SELECT * FROM kontrakan WHERE nik = '" . $_SESSION['nik'] . "'");
$row = mysqli_fetch_assoc($query);
echo "<input type='text' class='form-control' value='" . $row['pemilik'] . "' name='pemilik' readonly>
					";?>
						</div>
						<div class="form-row">
							<div class="form-group col-md-6">
								<label>pilih alamat kontrakan :</label>
								<select class='form-control' name='alamat_kontrakan'>
									<?php
include 'config/koneksi.php';
$query = mysqli_query($conn, "SELECT * FROM kontrakan WHERE nik = '" . $_SESSION['nik'] . "'");
while ($row = mysqli_fetch_assoc($query)) {
    echo "
            <option value='" . $row['alamat'] . ", RT" . $row['rt'] . "/RW" . $row['rw'] . "'>
							" . $row['alamat'] . ", RT" . $row['rt'] . "/RW" . $row['rw'] . "</option>
							";}?>
								</select>
							</div>
						</div>
